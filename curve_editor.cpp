...
// this is a function I used elsewhere in my project and I used it, you can swap it out if you dont want it
...
static const char* CurvePointStrs[] = {
	"Linear"
	"Constant"
	"SplitTangents"
	"Aligned"
};
static const int CurvePointStrsCount = 4;
...
void CurveEditorImgui::draw_editor_space()
{
    ...
    if (ImGui::BeginPopup("point_popup")) {
        if (selected_curve == -1 || selected_curve >= curves.size()) {
            ...
            if (ImGui::BeginCombo("##type", current_type.c_str())) {
                for (int i = 0; i < CurvePointStrsCount; i++) {
                    std::string str = CurvePointStrs[i];
    ...
    if (ImGui::BeginPopup("curve_edit_popup")) {
        if (selected_curve == -1 || selected_curve >= curves.size()) {
            printf("??? selected_curve null in curve_edit_popup\n");
            ImGui::CloseCurrentPopup();
...
void CurveEditorImgui::draw()
{
   ...
   if (ImGui::BeginPopup("curve_popup")) {
       if (selected_curve < 0 || selected_curve >= curves.size()) {
           printf("??? curve_popup bad selected_curve\n");
           set_selected_curve(-1);
           ImGui::CloseCurrentPopup();
       }
       else {
